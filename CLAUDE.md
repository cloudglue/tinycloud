...
TINYCLOUD_CMD=/path/to/tinycloud EXPECTED_VERSION=0.3.6 bash scripts/smoke-test.sh

# Serve a tarball as a fake CDN (modes: --corrupt, --no-manifest)
node test/fixtures/make-fixture-cdn.mjs --tarball <path>.tar.gz --version 0.3.6 --port 8787
...
node scripts/generate-manifest.mjs --version 0.3.6 --from-cdn   # build manifest + .sha256 sidecars
node scripts/generate-manifest.mjs --check --version 0.3.6      # verify live CDN matches manifest
...
because `--cached` only exists on watch/extract/caption/face/workflow). The
...
- The live CDN serves 0.3.6 (latest aliases + v-prefixed pinned tarballs
  for 0.3.0, 0.3.1, 0.3.2, 0.3.3, 0.3.4, 0.3.5, and 0.3.6, with `manifest.json` + `.sha256`
  sidecars; `channels.stable` = 0.3.6); all smoke legs are required.
