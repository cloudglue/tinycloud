...
TINYCLOUD_CMD=/path/to/tinycloud EXPECTED_VERSION=0.3.1 bash scripts/smoke-test.sh

# Serve a tarball as a fake CDN (modes: --corrupt, --no-manifest)
node test/fixtures/make-fixture-cdn.mjs --tarball <path>.tar.gz --version 0.3.1 --port 8787
...
node scripts/generate-manifest.mjs --version 0.3.1 --from-cdn   # build manifest + .sha256 sidecars
node scripts/generate-manifest.mjs --check --version 0.3.1      # verify live CDN matches manifest
...
- The live CDN serves 0.3.1 (latest aliases + v-prefixed pinned tarballs
  for 0.3.0 and 0.3.1, with `manifest.json` + `.sha256` sidecars;
  `channels.stable` = 0.3.1); all smoke legs are required.
