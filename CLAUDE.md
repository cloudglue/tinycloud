...
TINYCLOUD_CMD=/path/to/tinycloud EXPECTED_VERSION=0.3.0 bash scripts/smoke-test.sh

# Serve a tarball as a fake CDN (modes: --corrupt, --no-manifest)
node test/fixtures/make-fixture-cdn.mjs --tarball <path>.tar.gz --version 0.3.0 --port 8787
...
node scripts/generate-manifest.mjs --version 0.3.0 --from-cdn   # build manifest + .sha256 sidecars
node scripts/generate-manifest.mjs --check --version 0.3.0      # verify live CDN matches manifest
...
- Live-CDN jobs carry `continue-on-error: true` with `TODO(release-0.3.0)`
  markers: the live CDN still serves 0.2.3 and has no pinned tarballs or
  manifest. Remove the markers once 0.3.0 + manifest are uploaded.
