...
function isInstalled(version) {
  return fs.existsSync(path.join(versionDir(version), ".ok"));
}

function okMatchesTarget(okPath, target) {
  try {
    const meta = JSON.parse(fs.readFileSync(okPath, "utf8"));
    return (
      meta.target === target ||
      (!meta.target && typeof meta.url === "string" && meta.url.includes(`tinycloud-${target}`))
    );
  } catch {
    return false;
  }
...
function isInstalledForTarget(version, target) {
  return okMatchesTarget(path.join(versionDir(version), ".ok"), target);
}
...
function findInstalledByUrlEtag(url, etag) {
  ...
  for (const v of entries) {
    try {
      ...
      if (meta.url === url && meta.etag && meta.etag === etag) {
...
async function ensureInstalled(version, target) {
  ...
  if (version !== "latest") {
    ...
    if (isInstalledForTarget(version, target)) return { dir: dest, version };
  }
  ...
  if (res.version) {
    ...
    if (isInstalledForTarget(res.version, target)) return { dir: dest, version: res.version };
  } else {
  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(path.join(dest, ".ok")) && !isInstalledForTarget(version_, target)) {
