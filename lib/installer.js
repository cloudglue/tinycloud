 ...
 * removed and don't count against `keep`. Broken entries (a .ok marker but
 * no runnable binary) are removed outright and never occupy a retention
 * slot — otherwise a corrupted cache could crowd out working versions.
 */
function pruneVersions(keep = 2, protect = []) {
  ...
  for (const e of entries) {
    ...
    (isInstalled(e) ? healthy : broken).push(e);
  }
