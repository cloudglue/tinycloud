...
# Accept a leading v (v0.3.0 == 0.3.0)
VERSION="${VERSION#v}"
...
HAVE_MANIFEST=0
if curl -fsSL -o "$MANIFEST_FILE" "${BASE_URL}/manifest.json" 2>/dev/null; then
  HAVE_MANIFEST=1
fi

if [ "$HAVE_MANIFEST" -eq 0 ] && [ "${TINYCLOUD_REQUIRE_MANIFEST:-0}" = "1" ]; then
  echo "Error: TINYCLOUD_REQUIRE_MANIFEST=1 but ${BASE_URL}/manifest.json is missing" >&2
  exit 1
fi

# json_get FILE KEY... (each KEY is one object level; keys may contain dots,
# e.g. json_get m.json versions 0.3.0 platforms darwin-arm64 url) — python3
# when available, else a scoped sed fallback good enough for our manifest.
json_get() {
  local file="$1"
  shift
...
PYEOF
    ...
    local leaf=""
    for leaf in "$@"; do :; done
    sed -n "s/.*\"${leaf}\"[[:space:]]*:[[:space:]]*\"\{0,1\}\([^\",}]*\)\"\{0,1\}.*/\1/p" "$file" | head -1
  fi
}
